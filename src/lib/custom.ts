...
export async function ApplyCustom(reg: Register, options: any) {
	...
	if (FileExists(customConfigPath)) {
		...
		customConfig = (await ReadJsonFile(
			`${options.directory}/config/custom.json`
		)) as CustomConfig;
	}
...
function DontIgnore(filePath: string, customConfig: CustomConfig) {
	const found = customConfig.ignore.masks.find((el) =>
		new RegExp(el).test(filePath)
	);

	return found === undefined;
}

async function CopyNotFoundFile(
	oldFile: string,
	el: CopyData,
...
async function CustomNotFoundFile(
	el: RenderData,
	oldFile: string,
	customConfig: CustomConfig
) {
...
async function CustomExistsFile(
	el: RenderData,
	oldFile: string,
	newContent: string,
	customConfig: CustomConfig
